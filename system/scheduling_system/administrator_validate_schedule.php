            ...
            if (
                ...
                if (
                    ...
                    $conflicts[] = [
                        ...
                        'description' =>
                            "{$s1['classroom']} is used by {$s1['subject']} and {$s2['subject']} on {$s1['day']} with overlapping time.",
                        'schedule1' => $s1,
                        'schedule2' => $s2
                    ];
                }
...
<body>
    ...
    <aside class="sidebar">
        ...
        <nav id="sidebarNav">
            ...
            <a href="administrator_validate_schedule.php" class="active">Validate Schedule</a>
    ...
    <?php elseif (empty($conflicts)): ?>
        ...
        <div class="success-result">
            ...
            <p>
                ...
                <?php echo isset($schedules) ? count($schedules) : 0; ?>
                schedules are valid.
            </p>
        </div>
    ...
    <?php else: ?>
        ...
        <p class="schedule-count">
            Validated <?php echo isset($schedules) ? count($schedules) : 0; ?> schedules.
            Found <?php echo isset($conflicts) ? count($conflicts) : 0; ?> conflict(s):
        </p>
        ...
        <?php foreach ($conflicts as $conflict): ?>
            <div class="conflict-item">
                ...
                <small>
                    Schedule 1:
                    <?php echo htmlspecialchars($conflict['schedule1']['subject'] ?? 'N/A'); ?>
                    (
                    <?php echo formatTime($conflict['schedule1']['start_time'] ?? ''); ?>
                    -
                    <?php echo formatTime($conflict['schedule1']['end_time'] ?? ''); ?>
                    )
                    <br>

                    Schedule 2:
                    <?php echo htmlspecialchars($conflict['schedule2']['subject'] ?? 'N/A'); ?>
                    (
                    <?php echo formatTime($conflict['schedule2']['start_time'] ?? ''); ?>
                    -
                    <?php echo formatTime($conflict['schedule2']['end_time'] ?? ''); ?>
                    )
                </small>
            </div>
        <?php endforeach; ?>
